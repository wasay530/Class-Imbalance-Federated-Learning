...
from fedbench.engine.trainer import train_local, evaluate
from fedbench.engine.federated import get_weights, set_weights, fedavg
from fedbench.losses.ldam import LDAMLoss, compute_drw_weights, TinyHARWithNormalizedLogits
from fedbench.datasets.wear_dataset import make_train_val_split
from fedbench.losses.focal import focal_loss
from fedbench.losses.ratio import RatioLoss
from fedbench.utils.metrics import compute_metrics
...
def _local_class_counts(y_train, num_classes: int) -> np.ndarray:
    # 1 Extract label array properly
...
def run_method(name: str, clients, model_ctor, num_classes: int, cfg: dict, out_dir: str):
    ...
    for rnd in range(num_rounds):
        ...
        for cid in client_ids:
            ...
            # choose loss
            if name == "FedLDAM":
                # loss_fn, phase = _build_ldam(clients, num_classes, cfg, rnd)
